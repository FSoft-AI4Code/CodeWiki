        ...
        if not repo_url:
            ...
            if existing_job:
                ...
                if cached_docs and Path(cached_docs).exists():
                    ...
                    total_tokens = None
                    try:
                        metadata = file_manager.load_json(Path(cached_docs) / "metadata.json")
                        total_tokens = (
                            (metadata.get("statistics") or {}).get("token_usage") or {}
                        ).get("total_tokens")
                    except Exception:
                        pass
    ...
    async def serve_generated_docs(
        ...
        if job:
            ...
            if cached_docs and Path(cached_docs).exists():
                ...
                try:
                    metadata = file_manager.load_json(docs_path / "metadata.json")
                    job.total_tokens = (
                        (metadata.get("statistics") or {}).get("token_usage") or {}
                    ).get("total_tokens")
                except Exception:
