...
class BackgroundWorker:
            ...
            for job_id, job_data in data.items():
                ...
                if job_data.get("status") == "completed":
                    job = JobStatus(
                        ...
                        total_tokens=job_data.get("total_tokens"),
                    )
                    if job.total_tokens is None and job.docs_path:
                        job.total_tokens = self._get_docs_total_tokens(job.docs_path)
                    self.job_status[job_id] = job
            print(
    ...
    def _get_docs_total_tokens(self, docs_path: str):
        ...
        try:
            ...
            return None
        return ((metadata.get("statistics") or {}).get("token_usage") or {}).get("total_tokens")
                ...
                try:
                    ...
                    if job_id not in self.job_status:
                        self.job_status[job_id] = JobStatus(
                            ...
                            total_tokens=self._get_docs_total_tokens(cache_entry.docs_path),
                        )
        ...
        if self.ws_manager:
            try:
                progress_msg = ProgressMessage(
                    job_id=job_id, status=status, progress=progress, **kwargs
                )
    ...
    def _process_job(self, job_id: str):
        ...
        try:
            ...
            if cached_docs and Path(cached_docs).exists():
                ...
                job.total_tokens = self._get_docs_total_tokens(cached_docs)
                ...
                self._send_progress(
                    job_id, "completed", job.progress, total_tokens=job.total_tokens
                )
            ...
            if not GitRepoProcessor.clone_repository(
            ...
            job.total_tokens = doc_generator.total_token_usage.get("total_tokens")
            ...
            # Send completion progress
            self._send_progress(job_id, "completed", job.progress, total_tokens=job.total_tokens)
